...
write.table (submit[c("id","DEATHS")], sep=",", file = "c:/kaggle/data/submit5_SVM_train.csv", col.names=TRUE, row.names = FALSE)

submit <- as.data.frame(pred.test.glm)
submit$rownumber = 1:nrow(submit)
colnames(submit) <- c("DEATHS","id")
write.table (submit[c("id","DEATHS")], sep=",", file = "c:/kaggle/data/submit5_SVM_test.csv", col.names=TRUE, row.names = FALSE)
