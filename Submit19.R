...
miningcities <- read.csv(file ="c:/kaggle/data/MiningCities.csv")
...
#closest mining city
miningcity <- function(Long, Lat, miningcities){
  for (i in 1:nrow(miningcities)){
    if (i==1){
      d <- data.frame(earth.dist(Long, Lat
                                 ,miningcities[i,'Long']
                                 ,miningcities[i,'Lat']))
  ...
  names(d) <- as.character(1:nrow(miningcities))
  apply(d, 1, min)
}
...
train.source$min.mining <- miningcity(train.source$Long, train.source$Lat, miningcities)
test.source$min.mining <- miningcity(test.source$Long, test.source$Lat, miningcities)

for (i in 1:nrow(train.source)){
  if (train.source$min.mining[i] <= 30) {train.source$mining[i] <- 1} else {train.source$mining[i] <- 0}
}

for (i in 1:nrow(test.source)){
  if (test.source$min.mining[i] <= 30) {test.source$mining[i] <- 1} else {test.source$mining[i] <- 0}
}
...
train.source$maleratio = (train.source$Age_0_4_yr_M + train.source$Age_5_14_yr_M + train.source$Age_15_19_yr_M + train.source$Age_20_24_yr_M + train.source$Age_25_34_yr_M
                          + train.source$Age_35_44_yr_M + train.source$Age_45_54_yr_M + train.source$Age_55_64_yr_M + train.source$Age_65_74_yr_M
                          + train.source$Age_75_84_yr_M + train.source$Age_85ov_M) / (train.source$Age_0_4_yr_F + train.source$Age_5_14_yr_F + train.source$Age_15_19_yr_F + train.source$Age_20_24_yr_F + train.source$Age_25_34_yr_F
                                                                                      + train.source$Age_35_44_yr_F + train.source$Age_45_54_yr_F + train.source$Age_55_64_yr_F + train.source$Age_65_74_yr_F 
                                                                                      + train.source$Age_75_84_yr_F + train.source$Age_85ov_F + 1)

test.source$maleratio = (test.source$Age_0_4_yr_M + test.source$Age_5_14_yr_M + test.source$Age_15_19_yr_M + test.source$Age_20_24_yr_M + test.source$Age_25_34_yr_M
                         + test.source$Age_35_44_yr_M + test.source$Age_45_54_yr_M + test.source$Age_55_64_yr_M + test.source$Age_65_74_yr_M
                         + test.source$Age_75_84_yr_M + test.source$Age_85ov_M) / (test.source$Age_0_4_yr_F + test.source$Age_5_14_yr_F + test.source$Age_15_19_yr_F + test.source$Age_20_24_yr_F + test.source$Age_25_34_yr_F
                                                                                   + test.source$Age_35_44_yr_F + test.source$Age_45_54_yr_F + test.source$Age_55_64_yr_F + test.source$Age_65_74_yr_F 
                                                                                   + test.source$Age_75_84_yr_F + test.source$Age_85ov_F + 1)
...
fit.glm <- train(DEATHS ~ #AREA_SQKM
                   #+ Long
                   #+ Lat
                   Age_0_4_yr_M
                 + Age_0_4_yr_F
                 ...
                 , method = c('glm'), data = traindata, trControl = train_control)

#stop multithreading
#stopCluster(cl)

# run model on training data
#pred.train.glm <- caretEnsemble(fit.glm)
pred.train.glm <- predict(fit.glm, newdata = traindata)
pred.train.glm[pred.train.glm<0] <- 0

# run model on validation data and test data
pred.valid.glm <- predict(fit.glm, newdata = validdata)
pred.valid.glm[pred.valid.glm<0] <- 0
pred.test.glm <- predict(fit.glm, newdata = testdata)
pred.test.glm[pred.test.glm<0] <- 0

#glmprop <- 0.75
#rfprop <- 1 - glmprop

#pred.train.all <- (pred.train.glm*glmprop + pred.train.rf*rfprop)
#pred.valid.all <- (pred.valid.glm*glmprop + pred.valid.rf*rfprop)
#pred.test.all <- (pred.test.glm*glmprop + pred.test.rf*rfprop)

#pred.train.all[pred.train.all<0] <- 0
#pred.valid.all[pred.valid.all<0] <- 0
#pred.test.all[pred.test.all<0] <- 0
...
write.table (submit[c("id","DEATHS")], sep=",", file = "c:/kaggle/data/submit19.csv", col.names=TRUE, row.names = FALSE)