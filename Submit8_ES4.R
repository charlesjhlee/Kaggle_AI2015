...
ens_train <- data.frame("id" = traindata$id, "DEATHS" = traindata$DEATHS, "RLM" = pred.train.rlm, "GBM" = pred.train.gbm, "RF" = pred.train.rf, "SVM" = pred.train.svm)
ens_test <- data.frame("id" = testdata$id, "RLM" = pred.test.rlm, "GBM" = pred.test.gbm, "RF" = pred.test.rf, "SVM" = pred.test.svm)
ens_valid <- data.frame("id" = validdata$id, "DEATHS" = validdata$DEATHS, "RLM" = pred.valid.rlm, "GBM" = pred.valid.gbm, "RF" = pred.valid.rf, "SVM" = pred.valid.svm)

cl <- makeCluster(detectCores() - 1)
registerDoParallel(cl)
fit.ens <- train(DEATHS ~ RLM + GBM + RF + SVM, method = 'rlm', data = ens_valid, trControl = train_control)
stopCluster(cl)

#convert result to data frame and then create submission csv file
write.table (ens_train, sep=",", file = "c:/kaggle/data/ens_train.csv", col.names=TRUE, row.names = FALSE)
...
write.table (submit[c("id","DEATHS")], sep=",", file = "c:/kaggle/data/submit7.csv", col.names=TRUE, row.names = FALSE)
