...
set.seed(123)
train_ind <- createDataPartition(train.source$DEATHS, p=0.5, list=F)
traindata <- train.source[train_ind,]
validdata <- train.source[-train_ind,]
...
submit <- as.data.frame(pred.train.glm)
submit$rownumber = 1:nrow(submit)
colnames(submit) <- c("DEATHS","id")
write.table (submit[c("id","DEATHS")], sep=",", file = "c:/kaggle/data/submit5_NNET_train.csv", col.names=TRUE, row.names = FALSE)

submit <- as.data.frame(pred.test.glm)
submit$rownumber = 1:nrow(submit)
colnames(submit) <- c("DEATHS","id")
write.table (submit[c("id","DEATHS")], sep=",", file = "c:/kaggle/data/submit5_NNET_test.csv", col.names=TRUE, row.names = FALSE)
