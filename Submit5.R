...
SVM_train <- read.csv(file = "c:/kaggle/data/submit5_SVM_train.csv")
GBM_train <- read.csv(file = "c:/kaggle/data/submit5_GBM_train.csv")
RLM_train <- read.csv(file = "c:/kaggle/data/submit5_RLM_train.csv")
RF_train <- read.csv(file = "c:/kaggle/data/submit5_RF_train.csv")
NNET_train <- read.csv(file = "c:/kaggle/data/submit5_NNET_train.csv")

SVM_test <- read.csv(file = "c:/kaggle/data/submit5_SVM_test.csv")
GBM_test <- read.csv(file = "c:/kaggle/data/submit5_GBM_test.csv")
RLM_test <- read.csv(file = "c:/kaggle/data/submit5_RLM_test.csv")
RF_test <- read.csv(file = "c:/kaggle/data/submit5_RF_test.csv")
NNET_test <- read.csv(file = "c:/kaggle/data/submit5_NNET_test.csv")
...
set.seed(123)
train_ind <- createDataPartition(train.source$DEATHS, p=0.5)[[1]]
traindata <- as.data.table(train.source[train_ind, ])
validdata <- as.data.table(train.source[-train_ind, ])
testdata <- test.source

traindata$SVM_DEATHS <- SVM_train$DEATHS
traindata$GBM_DEATHS <- GBM_train$DEATHS
traindata$RLM_DEATHS <- RLM_train$DEATHS
traindata$RF_DEATHS <- RF_train$DEATHS
traindata$NNET_DEATHS <- NNET_train$DEATHS

testdata$SVM_DEATHS <- SVM_test$DEATHS
testdata$GBM_DEATHS <- GBM_test$DEATHS
testdata$RLM_DEATHS <- RLM_test$DEATHS
testdata$RF_DEATHS <- RF_test$DEATHS
testdata$NNET_DEATHS <- NNET_test$DEATHS
