...
set.seed(123)
train_ind <- createDataPartition(train.source$DEATHS, p=1, list=F)
...
fit.all <- train(DEATHS ~ #AREA_SQKM
                 ...
                 , method = 'brnn', data = traindata, trControl = train_control, neurons = sample(1:20, replace = TRUE, size = len))
...
write.table (submit[c("id","DEATHS")], sep=",", file = "c:/kaggle/data/submit5_BRNN.csv", col.names=TRUE, row.names = FALSE)
