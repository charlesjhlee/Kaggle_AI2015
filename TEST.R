...
train.source <- read.csv(file = "c:/kaggle/data/newtrainfew.csv")
test.source <- read.csv(file = "c:/kaggle/data/newtestfew.csv")
...
fit.rf <- train(DEATHS ~ F_BachDeg_Total
                +High_yr_schl_comp_Yr_12_eq_F
                +F_PGrad_Deg_Total
                +Count_Census_Nt_Ewhere_Aust_F
                +M_Tot_Volunteer
                ...
                +No_Religion_M
                     ...
                     , method = 'rf', data = traindata, trControl = train_control)
...
traindata$rf <- pred.train.rf
validdata$rf <- pred.valid.rf
testdata$rf <- pred.test.rf
